...
a).values}),0==json.tags.length||""==json.tags[0]?modalShow("Error","You need to select tags for your bot!"):request({url:"/api/users/"+context.user_id+"/bots/"+json.bot_id,method:method,userAuth:!0,json:json,statusCode:{200:function(){modalShow("Success","Your bot has been "+mod+". You will be redirected to it in a few moments");setTimeout(function(){window.location.replace("/bot/"+json.bot_id)},1E3)}}}))}catch(a){alert(a)}}
...
context.bot_id,method:"GET",statusCode:{404:function(b){window.location.replace("/")}}})},2E3),1E3)}}})):modalShow("Failed to delete bot","This bot couldn't be deleted as you did not confirm that you wanted to do this!")}
...
function postStats(){server_count=document.querySelector("#server-count").value;payload={guild_count:server_count};request({botAuth:!0,method:"POST",url:"/api/bots/"+context.bot_id+"/stats",json:payload});modalShow("Success","Done posting stats. You may leave this page or continue editing this bot!")}function regenToken(){request({botAuth:!0,method:"PATCH",url:"/api/bots/"+context.bot_id+"/token"});alert("Regenerated Token Successfully");window.location.reload()}
function hideSaveOnAboutTab(a,b,d){"about"==a||"actions"==a||"analytics"==a?document.querySelector("#save-changes").style.display="none":document.querySelector("#save-changes").style.display="initial"}var ws=null,cache={};
function listenAnalytics(){ws&&ws.close(1015);document.querySelector("#output-analytics").textContent="";ws=new FatesWS(context.bot_id,context.bot_token,!0,!1,!0);ws.hooks.event=function(a,b){var d,f,e,k,c,g;return $jscomp.asyncExecutePromiseGeneratorProgram(function(h){switch(h.nextAddress){case 1:if("global"==b.chan)return h.return();d=JSON.stringify(b.dat);console.log(Events,b.dat.m.e);f=b.dat.ctx.user;if("0"===f){f="Anonymous/Not logged in";h.jumpTo(2);break}if(userObj=cache[b.dat.ctx.user]){h.jumpTo(3);
break}return h.yield(fetch(context.site_url+"/api/users/"+b.dat.ctx.user+"/username"),4);case 4:return res=h.yieldResult,e=cache,k=b.dat.ctx.user,h.yield(res.json(),5);case 5:e[k]=h.yieldResult,userObj=cache[b.dat.ctx.user];case 3:f=userObj.username+" ("+b.dat.ctx.user+")";case 2:c=new Date(1E3*b.dat.m.ts),g=c+" (timestamp "+b.dat.m.ts+")",b.dat.m.e==Events.ViewEvent?d="User ("+f+") is viewing your bot at "+g:b.dat.m.e==Events.InviteEvent?d="User ("+f+") is interested in your bot and has clicked the 'Invite' button at "+
g:b.dat.m.e==Events.VoteEvent&&(d="User ("+f+") has voted for your bot and your bot now has "+b.dat.ctx.votes+" votes!"),document.querySelector("#output-analytics").textContent+="\n\n"+d,h.jumpToEnd()}})};ws.start()}
