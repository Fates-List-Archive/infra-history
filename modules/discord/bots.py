...
async def bot_settings(request: Request, bot_id: int):
    ...
    if (
        not check and bot_id != 798951566634778641
    ):  # Fates list main bot is staff viewable
        return await templates.e(
            request, "You are not allowed to edit this bot!", status_code=403
        )
    ...
    bot[
        "platform"
    ] = "discord"  # Will be removed once discord becomes de-facto platform
    ...
    bot["extra_owners"] = ",".join([str(o["owner"])
                                   for o in owners if not o["main"]])
    ...
    context = {
        "bot_token": await db.fetchval(
            "SELECT api_token FROM bots WHERE bot_id = $1", bot_id
        ),
        "mode": "edit",
        "bot_id": str(bot_id),
        ...
        "tags": [{"text": tag["name"], "value": tag["id"]} for tag in tags_fixed],
        "features": [
            {"text": feature["name"], "value": id} for id, feature in features.items()
        ],
        "votes": bot["votes"],
    }
...
@router.get("/{bot_id}")
async def bot_index(
    request: Request, bot_id: int, bt: BackgroundTasks, rev_page: int = 1
):
...
async def bot_review_page(
    request: Request, bot_id: int, page: int = 1, user_id: int | None = 0
):
    ...
    context = {
        "id": str(bot_id),
        "type": "bot",
        "reviews": {"average_rating": float(reviews[1])},
        "user_id": str(user_id),
    }
    data = {
    ...
    return await templates.TemplateResponse(
        "ext/reviews.html",
        {"request": request, "data": {"user": user}} | data,
        context=context,
    )
